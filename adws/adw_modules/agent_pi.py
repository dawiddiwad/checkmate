...
def _pi_catalog() -> list[tuple[str, str, int]]:
    ...
    if result.returncode != 0:
        return []
    # Installed pi (0.74.2) writes the --list-models table to stderr when
    # stdout isn't a TTY (e.g. under subprocess capture); older versions used
    # stdout. Take whichever stream actually has the table.
...
def context_window(provider: str, model_id: str) -> int:
    ...
    # models.json only exists once custom models have been registered with pi;
    # its absence just means "nothing beyond the built-in catalog below".
    try:
...
def run(request: PiRequest, on_event: Optional[Callable[[dict], None]] = None,
    ...
    # Installed pi (0.74.2) dropped `--session-id` for `--session <path|id>`,
    # and a bare id only *looks up* an existing session (errors if missing).
    # A full .jsonl path under session-dir still create-or-continues by id,
    # which is the behavior every ADW phase relies on.
    session_path = str(Path(request.session_dir) / f"{request.session_id}.jsonl")
    cmd = [
        PI_PATH, "-p", "--mode", "json",
