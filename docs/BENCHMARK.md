...
| Flow                     | checkmate cost | mcp-shaped cost | Ratio |
| ------------------------ | -------------- | --------------- | ----- |
...
`checkmate` also finished in fewer turns across the board. This was checked twice more, outside
this repo: once against a real `@playwright/mcp` server, which landed in the same range (roughly
2-4x more expensive, and noticeably slower); and once against a general-purpose coding agent
(Claude Code driving Playwright through its own CLI and built-in skills), which came out close to
