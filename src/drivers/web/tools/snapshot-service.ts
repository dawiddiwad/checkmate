...
export class SnapshotService {
	constructor(
		...
		private readonly step?: BrowserStepIntent,
	...
	private async compress(snapshot: string): Promise<string> {
		...
		const processed = shouldSkipFilter
			? snapshotTree
			: await filterSnapshot(snapshotTree, this.runtimeLogger, this.step, this.config.snapshotTopPercent)
	...
	async get(): Promise<BrowserSnapshot> {
		try {
			...
			this.runtimeLogger.debug(`created aria page snapshot:\n${compressedSnapshot}`)
