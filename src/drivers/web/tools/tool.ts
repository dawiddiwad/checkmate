...
import { AgentTool, AgentToolResponse } from '../../../tools/types.js'
...
import type { BrowserStepIntent, WebToolSettings } from './types.js'
...
export class BrowserToolRuntime {
	...
	constructor(
		page: Page,
		private readonly config: WebToolSettings,
	...
	async selectPage(pageId: string, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
	...
	async closePage(pageId: string | null | undefined, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
		const pages = await this.openPages()
		if (pages.length <= 1) {
	...
	async navigateToUrl(url: string, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
	...
	async clickElement(ref: string, hover: boolean, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
		return this.wrapWithTracker(
			async (page) => {
				try {
					if (hover) {
					...
					this.runtimeLogger.error(`error clicking element with ref '${ref}' due to:\n${error}`)
	...
	async uploadFiles(ref: string, filePaths: string[], step: BrowserStepIntent): Promise<AgentToolResponse | string> {
		return this.wrapWithTracker(
			async (page) => {
				try {
					...
					this.runtimeLogger.error(`error uploading files to element with ref '${ref}' due to:\n${error}`)
	...
	async pressKey(key: string, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
		return this.wrapWithTracker(
			async (page) => {
				try {
					if (!key) {
					...
					this.runtimeLogger.error(`error pressing key '${key}' due to:\n${error}`)
	...
	async captureCurrentSnapshot(step: BrowserStepIntent, options: { skipFilter?: boolean } = {}): Promise<string> {
		const page = await this.ensureActivePage()
		return new SnapshotService(page, this.config, this.runtimeLogger, step, {
			skipFilter: options.skipFilter,
		}).get()
	}

	async wait(seconds: number, step: BrowserStepIntent): Promise<AgentToolResponse | string> {
		return this.wrapWithTracker(
			async (page) => {
				try {
					...
					this.runtimeLogger.error(`error waiting for ${seconds} seconds due to:\n${error}`)
