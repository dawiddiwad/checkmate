import type { ConsoleMessage, Dialog, Frame, Page } from 'playwright'
...
const MUTATION_QUEUE_KEY = '__checkmate_mutation_queue'
...
class MutationObserverController {
	constructor(
		...
		private readonly recordMutation: (message: string) => void
	) {}

	async attach(): Promise<void> {
		await this.page
			.evaluate(setupMutationObserver, { observerKey: MUTATION_OBSERVER_KEY, queueKey: MUTATION_QUEUE_KEY })
			.catch(() => {})
	}

	async detach(): Promise<void> {
		await this.page
			.evaluate(teardownMutationObserver, { observerKey: MUTATION_OBSERVER_KEY, queueKey: MUTATION_QUEUE_KEY })
			.then((messages) => messages.forEach(this.recordMutation))
			.catch((error) => {
				this.runtimeLogger.debug(
					`failed to remove MutationObserver from page due to error:\n${JSON.stringify(error, null, 2)}`
				)
			})
	}
}
...
type ObserverSetupArgs = { observerKey: string; queueKey: string }

function setupMutationObserver({ observerKey, queueKey }: ObserverSetupArgs): void {
	const win = window as unknown as Window & Record<string, MutationObserver | string[] | undefined>
	const skipTags = new Set(['INS', 'IFRAME', 'SCRIPT', 'STYLE'])

	const existingObserver = win[observerKey]
	...
	const observer = new MutationObserver((mutations) => {
		...
		if (uniqueAppeared.length > 0) {
			queue.push(`Appeared: ${uniqueAppeared.join(' | ').substring(0, 300)}`)
		}

		if (uniqueDisappeared.length > 0) {
			queue.push(`Disappeared: ${uniqueDisappeared.join(' | ').substring(0, 300)}`)
		}

		if (uniqueAttributeChanges.length > 0) {
			queue.push(`Changed: ${uniqueAttributeChanges.join(' | ').substring(0, 400)}`)
		}
	})
...
function teardownMutationObserver({ observerKey, queueKey }: ObserverSetupArgs): string[] {
	const win = window as unknown as Window & Record<string, MutationObserver | string[] | undefined>
	const observer = win[observerKey]
	if (observer instanceof MutationObserver) {
		observer.disconnect()
	}
	const messages = win[queueKey]
...
export class TransientStateTracker {
	...
	constructor(
		...
		const runtimeLogger = normalizedOptions.logger ?? silentLogger

		this.timeline = new TimelineRecorder(clock)
		this.mutationObserver = new MutationObserverController(page, runtimeLogger, (message) =>
			this.timeline.recordUnique(message)
		)
