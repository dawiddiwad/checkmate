...
export class SalesforceCliHandler {
	...
	private ignored(error: string): boolean {
		return ['debugger', 'deprecation'].some((message) => error.includes(message))
	}
	...
	async runCommand({ command, flags, log }: CliCommand): Promise<Record<string, unknown> | string> {
		...
		return new Promise<Record<string, unknown> | string>((resolve, reject) => {
			exec(compiledArguments, (error, stdout, stderr) => {
				try {
					...
					if (stderr && !this.ignored(stderr)) {
						throw new Error(`command failed\nError details:${stderr}`)
					}

					if (!stdout) {
						throw new Error(`missing output from ${this.defaultPath} cli command`)
					}
