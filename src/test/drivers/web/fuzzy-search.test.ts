...
import {
	...
	filterTopPercent,
	JsonValue,
} from '../../../drivers/web/tools/snapshot-filter/semantic-scorer'
import { reconstructTree } from '../../../drivers/web/tools/snapshot-filter/tree-reconstructor'
import { filterSnapshot } from '../../../drivers/web/tools/snapshot-filter/snapshot-filter'
import type { BrowserStepIntent as Step } from '../../../drivers/web/tools/types'
	...
	describe('Snapshot Filter', () => {
		describe('filterSnapshot', () => {
			it('should return original JSON when step is undefined', async () => {
				const json = { key: 'value' }
				expect(await filterSnapshot(json, silentLogger, undefined)).toEqual(json)
			})
			...
			it('should prefer step.search over semantic action and expectation when provided', async () => {
				...
				const result = (await filterSnapshot(json, silentLogger, step)) as JsonValue
			...
			it('should handle the example from the spec with elements', async () => {
				...
				const result = await filterSnapshot(inputSnapshot, silentLogger, step)
				const resultStr = JSON.stringify(result)
