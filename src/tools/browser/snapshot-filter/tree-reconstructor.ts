...
function collectAllRequiredPaths(elements: ScoredElement[]): Set<string> {
	...
	for (const element of elements) {
		for (let i = 0; i <= element.path.length; i++) {
			paths.add(JSON.stringify(element.path.slice(0, i)))
		}

		paths.add(JSON.stringify(element.path))
	}
...
function isAncestorOf(potentialAncestor: (string | number)[], descendant: (string | number)[]): boolean {
	return (
		potentialAncestor.length < descendant.length &&
		potentialAncestor.every((segment, index) => segment === descendant[index])
	)
}
...
function buildFilteredArray(
	...
	for (let index = 0; index < value.length; index++) {
		const filteredChild = buildFilteredTree(value[index], requiredPaths, [...currentPath, index])
	...
	if (filteredItems.length === 0) {
		return includeDirectChildren(requiredPaths, currentPath) ? value : undefined
	}
...
function buildFilteredObject(
	...
	for (const key of Object.keys(value)) {
		const filteredChild = buildFilteredTree(value[key], requiredPaths, [...currentPath, key])
		if (filteredChild !== undefined) {
	...
	if (!hasContent && includeDirectChildren(requiredPaths, currentPath)) {
		return value
	}
...
function includeDirectChildren(requiredPaths: Set<string>, currentPath: (string | number)[]): boolean {
