...
import { ToolCall, ToolCallResult, ToolCallState, ToolContract, ToolExecutionContext } from '../tool-contract'
...
export class BrowserTool extends ToolContract {
		...
		this.functionDeclarations = [
			...
			},
			{
				...
				function: {
					...
					description: 'Capture the ARIA snapshot of the current page - use only if you miss information from previous snapshot.',
					parameters: {
						type: 'object',
						properties: {
			...
			},
			{
				...
				function: {
					...
					parameters: {
						type: 'object',
						properties: {
							seconds: { type: 'number', description: 'Number of seconds to wait, example: 5' },
	...
	async call(specified: ToolCall): Promise<string> {
		const result = await this.executeWithState(specified)
	...
	async execute(specified: ToolCall, _context: ToolExecutionContext): Promise<ToolCallResult> {
		return this.executeWithState(specified)
	}
	...
	async executeWithState(specified: ToolCall): Promise<ToolCallState | string> {
		if (!specified.name) {
		...
		if (specified.name === BrowserTool.TOOL_SNAPSHOT) {
			return {
		...
		if (specified.name === BrowserTool.TOOL_CLICK_OR_HOVER) {
			return this.clickElement(specified.arguments?.ref as string, specified.arguments?.hover as boolean)
		}

		if (specified.name === BrowserTool.TOOL_TYPE_OR_SELECT) {
			return this.typeOrSelectInElement(specified.arguments?.elements as BrowserInputElement[])
		}

		if (specified.name === BrowserTool.TOOL_PRESS_KEY) {
			return this.pressKey(specified.arguments?.key as string)
		}

		if (specified.name === BrowserTool.TOOL_WAIT) {
	...
	private async captureSnapshot(options: { skipFilter?: boolean } = {}) {
		try {
			...
			return new SnapshotService(this.page, this.step, { skipFilter: options.skipFilter }).get()
		} catch (error) {
	...
	private async typeOrSelectInElement(elements: BrowserInputElement[]) {
		return this.wrapWithTracker(
			async () => {
